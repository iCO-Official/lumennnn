    ...
    <section className="fixed inset-x-0 bottom-[calc(4.5rem+env(safe-area-inset-bottom))] top-0 z-20 mx-auto flex max-w-4xl flex-col bg-background pt-[env(safe-area-inset-top)]">
      ...
      <Conversation className="min-h-0">
        <ConversationContent className="mx-auto w-full max-w-3xl gap-5 px-4 py-6">
          {loading ? (
            <p className="py-16 text-center text-sm text-muted-foreground">Загружаю разговор…</p>
          ) : messages.length === 0 ? (
            <ConversationEmptyState
              icon={<Sparkles className="h-8 w-8" />}
              title="Привет, я рядом"
              description="Попроси поставить задачу, составить план или перенести дело."
            />
          ) : (
          ...
          {sending && (
            <Message from="assistant">
              <MessageContent className="text-muted-foreground">Думаю…</MessageContent>
            </Message>
          )}
        </ConversationContent>
        <ConversationScrollButton />
          ...
          <PromptInputFooter className="justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-muted-foreground">
                AI сначала предложит изменения
              </span>
              <PromptInputSubmit
